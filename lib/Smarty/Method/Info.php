...
class Smarty_Method_Info
{
    ...
    protected function analyzePropertiesPlausibility()
    {
        ...
        if ($smarty->securityPolicy && $smarty->securityPolicy->php_handling != $smarty->php_handling) {
    ...
    protected function analyzePlugins()
    {
        ...
        foreach ($directories as $dir) {
            ...
            foreach ($iterator as $file) {
                ...
                $autoload = false;
                // _autoloadFilters
                if (isset($type[6]) && substr($type, - 6) == 'filter' && $smarty->_autoloadFilters) {
                    $filter = substr($type, 0, - 6);
                    if (isset($smarty->_autoloadFilters[$filter]) && in_array($name, $smarty->_autoloadFilters[$filter])) {
                        $autoload = true;
                    }
                }

                // _defaultModifier
                if ($type == 'modifier' && $smarty->_defaultModifier && in_array($name, $smarty->_defaultModifier)) {
                    $autoload = true;
                }
    ...
    protected function analyzeDefaults()
    {
        ...
        foreach ($smarty->_defaultModifier as $callback) {
            $function = $this->reflectedFunctionOfCallable($callback);
        }
    ...
    protected function analyzeSecurity()
    {
        ...
        if ($smarty->securityPolicy) {
            $smarty = $smarty->securityPolicy;
