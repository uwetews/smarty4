 ...
 * @package Compiler
 */
class Smarty_Compiler_Php_NodeCompiler_Tag_Foreach extends Smarty_Exception_Magic
{

    /**
     * Compile {foreach} and {foreachelse} tags
     *
    ...
    public static function compile(Smarty_Compiler_Node $target, Smarty_Source_Node_Tag $node, $delete)
    {
        switch ($node->tag) {
            case 'foreach':
                self::compile_foreach($target, $node, $delete);
                break;
            case 'foreachelse':
                self::compile_foreachelse($target, $node, $delete);
                break;
            default:
                // TODO Expection
        }
     ...
     * @param bool                   $delete
     */
    public static function compile_foreach(Smarty_Compiler_Node $target, Smarty_Source_Node_Tag $node, $delete)
    {
        ...
        if (isset($node->attributeNodes['key'])) {
            $key = trim($compiling_node->compileNode($node->attributeNodes['key'])
                            ->getFormated(), "'\"");
            unset($node->attributeNodes['key']);
            $target->code("\$_scope->_tpl_vars->{$key} = new Entry;")
                ->newline();
        } else {
            $key = null;
        }
        $target->code('$_from = ');
        $target->compileNode($node->attributeNodes['from']);
        $target->raw(";\n")
        ...
        $target->code("foreach (\$_from as " . $keyterm . " \$_scope->_tpl_vars->{$item}->value) {\n")
            ->indent()
     ...
     * @param bool                   $delete
     */
    public static function compile_foreachelse(Smarty_Compiler_Node $target, Smarty_Source_Node_Tag $node, $delete)
    {
        $target->lineNo($node->sourceLineNo)
