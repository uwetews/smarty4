 ...
 * @package Smarty\Compiler\PHP\Tag
 */
class Smarty_Compiler_Php_NodeCompiler_Tag_Assign
{
    ...
    public static function compile(\Smarty_Compiler_Node $target, Smarty_Source_Node_Tag $node, $delete = true)
    {
        ...
        /**
         * if (! ($node->attributeNodes['var'] instanceof Smarty_Source_Node_Value_Expression)) {
         * $node->attributeNodes['var']->compileAsValue = true;
         * $target->code('$_scope->_tpl_vars->');
         * } else {
         * $target->code('');
         * }
         * */
        $target->code('$_scope->_tpl_vars->{');
        $target->compileNode($node->attributeNodes['var'])
            ->raw('} = new Entry(')
            ->compileNode($node->attributeNodes['value'], $delete)
            ->raw(");\n");
    }
