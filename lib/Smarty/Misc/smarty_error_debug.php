...
class Smarty_Error_Debug extends Smarty
{
    ...
    protected static $ignored_properties = array(
        ...
        'config_vars'      => true,
        // working these extra
        '_defaultModifier' => true,
        '_autoloadFilters' => true,
        // working these extra
        'template_dir'     => true,
        'compile_dir'      => true,
        'config_dir'       => true,
    ...
    */
    ...
    protected function analyzePropertiesPlausibility()
    {
        ...
        if ($this->smarty->securityPolicy && $this->smarty->securityPolicy->php_handling != $this->smarty->php_handling) {
    ...
    protected function analyzePlugins()
    {
        ...
        foreach ($directories as $dir) {
            ...
            foreach ($iterator as $file) {
                ...
                $autoload = false;
                // _autoloadFilters
                if (isset($type[6]) && substr($type, - 6) == 'filter' && $this->smarty->_autoloadFilters) {
                    $filter = substr($type, 0, - 6);
                    if (isset($this->smarty->_autoloadFilters[$filter]) && in_array($name, $this->smarty->_autoloadFilters[$filter])) {
                        $autoload = true;
                    }
                }

                // _defaultModifier
                if ($type == 'modifier' && $this->smarty->_defaultModifier && in_array($name, $this->smarty->_defaultModifier)) {
                    $autoload = true;
                }
     ...
     * @param $filepath
     *
     * @return null|ReflectionClass|ReflectionFunction|string
     */
    ...
    protected function analyzeDefaults()
    {
        ...
        foreach ($this->smarty->_defaultModifier as $callback) {
            $function = $this->reflectedFunctionOfCallable($callback);
        }
        // Smarty::$_autoloadFilters might be getting an overhaul
    }
    ...
    protected function analyzeSecurity()
    {
        ...
        if ($this->smarty->securityPolicy) {
            $smarty = $this->smarty->securityPolicy;
