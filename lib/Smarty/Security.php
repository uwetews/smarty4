...
class Smarty_Security
{
     ...
     * @var array
     */
    protected $_Code_Php_resource_dir = null;
    /**
     ...
     */
    public static function enableSecurity($smarty, $securityClass)
    {
        if ($securityClass instanceof Smarty_Security) {
            $smarty->securityPolicy = $securityClass;

            return false;
        } elseif (is_object($securityClass)) {
            throw new Smarty_Exception("enableSecurity(): Class '" . get_class($securityClass) . "' must extend Smarty_Security.");
        }
        if ($securityClass == null) {
            $securityClass = $smarty->securityClass;
        }
        if (!class_exists($securityClass)) {
            throw new Smarty_Exception("enableSecurity(): Security class '$securityClass' is not defined");
        } elseif ($securityClass !== 'Smarty_Security' && !is_subclass_of($securityClass, 'Smarty_Security')) {
            throw new Smarty_Exception("enableSecurity(): Class '$securityClass' must extend Smarty_Security.");
        } else {
            $smarty->securityPolicy = new $securityClass($smarty);
        }
    ...
    public function isTrustedPHPDir($filepath)
    {
        ...
        if (!$this->_trusted_dir || $this->_trusted_dir !== $this->trusted_dir) {
            $this->_Code_Php_resource_dir = array();

            $this->_trusted_dir = $this->trusted_dir;
            foreach ((array) $this->trusted_dir as $directory) {
                $directory = $this->realpath($directory);
                $this->_Code_Php_resource_dir[$directory] = true;
            }
        }
        ...
        while (true) {
            ...
            // test if the directory is trusted
            if (isset($this->_Code_Php_resource_dir[$directory])) {
                // merge sub directories of current $directory into _resource_dir to speed up subsequent lookups
                $this->_Code_Php_resource_dir = array_merge($this->_Code_Php_resource_dir, $_directory);
