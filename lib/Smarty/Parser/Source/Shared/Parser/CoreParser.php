...
class CoreParser extends PegParser
{
    ...
    public function matchNodeOpenP($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['OpenP2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['OpenP2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['OpenP2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['OpenP2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodeOpenB($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['OpenB2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['OpenB2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['OpenB2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['OpenB2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodeOpenC($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['OpenC2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['OpenC2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['OpenC2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['OpenC2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodeCloseP($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['CloseP2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['CloseP2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['CloseP2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['CloseP2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodeCloseB($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['CloseB2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['CloseB2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['CloseB2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['CloseB2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodeCloseC($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['CloseC2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['CloseC2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['CloseC2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['CloseC2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodeEqual($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['Equal2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['Equal2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['Equal2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['Equal2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodeDollar($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['Dollar2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['Dollar2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['Dollar2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['Dollar2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodeHatch($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['Hatch2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['Hatch2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['Hatch2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['Hatch2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodeComma($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['Comma2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['Comma2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['Comma2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['Comma2'][$match[0][1]] = $subres;
                }
            } else {
    ...
    public function matchNodePtr($previous, &$errorResult)
    {
        ...
        if (isset($this->parser->regexpCache['Ptr2'][$pos])) {
            ...
            if (empty($this->parser->regexpCache['Ptr2']) && preg_match_all($regexp . 'Sx', $this->parser->source, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER, $pos)) {
                $this->parser->regexpCache['Ptr2'][- 1] = true;
                foreach ($matches as $match) {
                    $subres = array('_silent' => 0, '_text' => $match[0][0], '_startpos' => $match[0][1], '_endpos' => $match[0][1] + strlen($match[0][0]), '_matchres' => array());
                    $this->parser->regexpCache['Ptr2'][$match[0][1]] = $subres;
                }
            } else {
