...
class Generator extends Parser
{
    ...
    /**
     *
     * Parser generated on 2014-06-18 02:51:35
     *  Rule filename 'C:\wamp\www\smarty4\lib\Smarty/Tool/Parser/Peg/InternalTool/ParserGenerator.peg.inc' dated 2014-06-18 00:24:16
     *
    */
    ...
    public $rules = array(
            ...
            "Node" => array(
                    ...
                    "_param" => array(
                            ...
                            1 => array(
                                    "_param" => "/\\s*\\<(?<type>(node|token))\\s+(?<nodename>[a-zA-Z_0-9]+)\\>/",
                                    "_type" => "rx"
                                ),
                            2 => array(
                                    "_max" => null,
            ...
            "RuleToken" => array(
                    ...
                    "_actions" => array(
                            "_start" => array(
                                    "RuleToken___START" => true
                                ),
                            "_match" => array(
                                    "Sequence" => array(
                                            "RuleToken_Sequence" => true
                                        )
                                ),
                            "_finish" => array(
                                    "RuleToken___FINISH" => true
                                )
                        )
                ),
            "File" => array(
    ...
    */
     ...
     *
     *  Rule:
     <node Parser> <rule>  ..Header .._? '<pegparser' _ Name '>' Attribute* Node* .._? '</pegparser>' .._? End? </rule>  <action _start> {
    ...
    */
     ...
     *
     *  Rule:
     <node Attribute> <rule>  .._? '<attribute>' attr '</attribute>' .._? </rule>  <action attr> {
    ...
    */
     ...
     *
     *  Rule:
     <node Node> <rule>  .._? /\s*\<(?<type>(node|token))\s+(?<nodename>[a-zA-Z_]+)\>/ Attribute* Rule .Action* /<\/(node|token)>/ .._? </rule>  <action type> {
    ...
    */
     ...
     *
     *  Rule:
     <node Rule> <rule>  .._? '<rule>' .._? Sequence .._? '</rule>' .._? </rule>  <action Sequence> {
    ...
    */
     ...
     *
     *  Rule:
     <node RuleToken> <rule>  /((?<silent>\.+)|(?<pla>&)|(?<nla>\!))?((?<tag>\w+):)?/ (  /(?<rx>\G(\/|~|@|%|§)(((\\\\)*\\\2)|.*?(?=(\\|\2)))*\2)|((?<osp>_\?)|(?<wsp>_))|(?<node>\w+)|(?<literal>("[^"]*")|('[^']*'))|(\$(?<expression>\w+))/ | (  '(' .._? Sequence .._? ')' ) ) /((?<quest>\?)|(?<any>\*)|(?<must>\+?)|(\{(?<min>\d+)?,(?<max>\d+)?\}))?/ </rule>  <action _start> {
                        $result['_rule'] = array();
                    } </action>  <action Sequence> {
                        $result['_rule'] = $subres['_rule'];
                    } </action>  <action _finish> {
    ...
    */
    ...
    public function RuleToken_Sequence (&$result, $subres) {
        $result['_rule'] = $subres['_rule'];
    }
