...
*/
%name CONFIG_
%declare_class {class Smarty_Compiler_Source_Language_Config_Parser extends Smarty_Compiler_Parser}
%include_class
{
    ...
    private function set_var(Array $var, Array &$target_array) {
        ...
        if ($this->context->smarty->config_overwrite || !isset($target_array['vars'][$key])) {
    ...
    private function add_global_vars(Array $vars) {
        if (!isset($this->currentNode->config_variables['vars'])) {
      $this->currentNode->config_variables['vars'] = Array();
        }
        foreach ($vars as $var) {
            $this->set_var($var, $this->currentNode->config_variables);
        }
    }

    private function add_section_vars($section_name, Array $vars) {
        if (!isset($this->currentNode->config_variables['sections'][$section_name]['vars'])) {
            $this->currentNode->config_variables['sections'][$section_name]['vars'] = Array();
        }
        foreach ($vars as $var) {
            $this->set_var($var, $this->currentNode->config_variables['sections'][$section_name]);
        }
    }
}


%token_prefix CONFIG_
...
section(res) ::= OPENB DOT SECTION(i) CLOSEB newline var_list(vars). {
    if ($this->context->smarty->config_read_hidden) {
        $this->add_section_vars(i, vars);
    }
    res = null;
}
